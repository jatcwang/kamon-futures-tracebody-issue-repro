...
import kamon.Kamon
import kamon.instrumentation.futures.scala.ScalaFutureInstrumentation.{Settings, startedSpan, traceBody}
...
with BeforeAndAfter{
  ...
  "traceBodyClose" in {
    ...
    val f = Kamon.runWithSpan(newSpan, finishSpan = false) {
      for {
        _ <- Future {
          traceBodyClose(SPAN1)(println("1"))
        }.map { _ =>
          traceBody(SPAN0 + "x1")
        }
        _ <- Future {
          traceBodyClose(SPAN2)(println("2"))
        }
        _ <- Future {
          traceBodyClose(SPAN3)(println("2"))
        }
      } yield newSpan.finish()
    }
